...
export function DirectorDetail({ director, onClose }: DirectorDetailProps) {
  ...
  const works = [
    ...
    { key: '特色佳作', icon: <Gem className="w-4 h-4" />, work: director.hiddenGem, color: 'text-purple-400', bg: 'bg-purple-500/10' },
  ...
  return (
    <AnimatePresence>
      {director && (
        ...
        >
          <motion.div
            className="w-full max-w-lg bg-zinc-900 border border-zinc-800 rounded-t-3xl sm:rounded-3xl max-h-[85vh] overflow-y-auto"
          ...
          >
            {/* Header */}
            <div className="sticky top-0 bg-zinc-900 z-10 flex items-center justify-between gap-4 p-6 border-b border-zinc-800">
                ...
                <div className="space-y-4">
                  {works.map(({ key, icon, work, color, bg }) => (
                    <div key={key} className={`${bg} rounded-xl p-4 border border-transparent`}>
                      ...
                      <div className="text-xs text-zinc-400 mt-1 leading-relaxed">{work.description}</div>
                      <div className="flex flex-wrap gap-2 mt-3">
                        {work.vibes.map((v) => (
                          <span
                            key={v}
                            className="px-2.5 py-1 rounded-full text-xs font-medium bg-zinc-800 text-zinc-400"
                          >
                            {v}
                          </span>
                        ))}
